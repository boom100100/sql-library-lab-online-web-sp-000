...
def select_series_title_with_most_human_characters
  "SELECT series.title, characters.species, COUNT(characters.species) FROM characters JOIN character_books ON characters.id = character_books.character_id JOIN books ON character_books.book_id = books.id JOIN series ON books.series_id = series.id GROUP BY series.title, characters.species ORDER BY COUNT(characters.species) DESC LIMIT 1;"
end
